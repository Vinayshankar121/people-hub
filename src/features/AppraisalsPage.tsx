...
function EmployeeAppraisals() {
  ...
  const confirmDeleteAppraisal = async () => {
    ...
    setDeleting(true);
    try {
      await deleteAppraisalRecord({ id: deleteTarget.id });
      toast.success("Appraisal deleted successfully");
      setDeleteTarget(null);
      await loadData();
    } catch (e: any) {
      toast.error(e?.message ?? "Failed to delete appraisal");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="space-y-6 sm:space-y-8 max-w-7xl">
                  ...
                  <tr key={app.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => setDeleteTarget({ id: app.id, name: app.appraisal_cycle })}
                          title="Delete Appraisal"
                          className="p-1.5 rounded-xl border border-slate-200 text-slate-400 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition"
                        >
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    ...
                    >
                      <Star
                        className={`h-8 w-8 ${star <= selfRating ? "fill-amber-400 text-amber-500" : "text-slate-300"
                          }`}
                      />
                    </button>
                  ))}
      ...
      >
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-3.5 bg-rose-50 border border-rose-100 rounded-xl text-rose-700 text-sm">
            <AlertTriangle className="h-5 w-5 shrink-0 mt-0.5" />
            <div>
              <p className="font-semibold text-rose-800">Delete Appraisal Record</p>
              <p className="mt-1 text-xs text-rose-600">
                Are you sure you want to delete the appraisal record for <strong>"{deleteTarget?.name}"</strong>? This action cannot be undone.
              </p>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t">
            <button
              ...
              className="px-4 py-2 rounded-xl text-xs font-semibold border text-slate-700 hover:bg-slate-50 transition"
            >
              Cancel
            </button>
            <button
              ...
              disabled={deleting}
              className="px-4 py-2 rounded-xl text-xs font-semibold bg-rose-600 text-white hover:bg-rose-700 disabled:opacity-50 transition"
            >
...
function AdminAppraisals() {
  ...
  // Delete modal state
  const [deleteTarget, setDeleteTarget] = useState<{ type: "cycle" | "appraisal"; id: string; name: string } | null>(null);
  const [deleting, setDeleting] = useState(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      if (deleteTarget.type === "cycle") {
        await deleteAppraisalCycle({ id: deleteTarget.id });
            ...
            {cycles.map((c) => (
              <div key={c.id} className="border rounded-2xl p-4 flex flex-col justify-between bg-slate-50/50">
                ...
                <div className="flex items-center justify-between mt-4 border-t pt-3">
                  <span className={`text-xs px-2 py-0.5 rounded-md font-semibold border ${c.status === "Active" ? "bg-emerald-50 text-emerald-700 border-emerald-200" :
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => setDeleteTarget({ type: "cycle", id: c.id, name: c.name })}
                      title="Delete Cycle"
                  ...
                  <tr key={app.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => setDeleteTarget({ type: "appraisal", id: app.id, name: app.employee_name || "Appraisal Record" })}
                          title="Delete Appraisal"
                          className="p-1.5 rounded-xl border border-slate-200 text-slate-400 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition"
                        >
                          <Trash2 className="h-4 w-4" />
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star
                          className={`h-8 w-8 ${star <= adminRating ? "fill-amber-400 text-amber-500" : "text-slate-300"
                            }`}
                        />
                      </button>
                    ))}
          ...
          <div className="flex items-start gap-3 p-3.5 bg-rose-50 border border-rose-100 rounded-xl text-rose-700 text-sm">
            ...
            <div>
              <p className="font-semibold text-rose-800">
                Delete {deleteTarget?.type === "cycle" ? "Appraisal Cycle" : "Appraisal Record"}
              </p>
              <p className="mt-1 text-xs text-rose-600">
                  ...
                  <tr key={app.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => setDeleteTarget({ id: app.id, name: app.employee_name || "Appraisal Record" })}
                          title="Delete Appraisal"
                          className="p-1.5 rounded-xl border border-slate-200 text-slate-400 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-200 transition"
                        >
                          <Trash2 className="h-4 w-4" />
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-3.5 bg-rose-50 border border-rose-100 rounded-xl text-rose-700 text-sm">
            ...
            <div>
              <p className="font-semibold text-rose-800">Delete Appraisal Record</p>
              <p className="mt-1 text-xs text-rose-600">
                Are you sure you want to delete the appraisal for <strong>"{deleteTarget?.name}"</strong>? This action cannot be undone.
              </p>
            </div>
          </div>
